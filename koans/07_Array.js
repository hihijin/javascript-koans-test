describe('Array에 대해서 학습합니다.', function () {
  ...
  it('Array의 요소(element)를 다루는 방법을 확인합니다.', function () {
    ...
    const poppedValue = arr.pop(); //없애는 배열의 요소!
    expect(poppedValue).to.equal(3); //3이 없애는 요소니까 3이 poppedValue의 값이 된다.
    expect(arr).to.deep.equal([1, 2]);
  });
  ...
  it('Array 메소드 slice를 확인합니다.', function () {
    ...
    expect(arr.slice(2, 2)).to.deep.equal([]); //인덱스2부터인덱스2전까지 -> 비워짐
    expect(arr.slice(2, 20)).to.deep.equal(['and', 'jelly']);
    expect(arr.slice(3, 0)).to.deep.equal([]); //인덱스 3부터 0전까지 -> 비워짐
    ...
    expect(arr.slice(0)).to.deep.equal(['peanut', 'butter', 'and', 'jelly']); //인덱스 0부터 끝까지
  });
