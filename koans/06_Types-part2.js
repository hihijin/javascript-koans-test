 ...
 */
  ...
  it('원시 자료형을 변수에 할당할 경우, 값 자체의 복사가 일어납니다.', function () {
    ...
    overTwenty = false;
    expect(overTwenty).to.equal(false); //재할당이 일어남
    expect(allowedToDrink).to.equal(true); // 원래의 값 복사
  ...
  */
  it('참조 자료형의 데이터는 동적(dynamic)으로 변합니다.', function () {
    const arr = [1, 2, 3];
    expect(arr.length).to.equal(3); //배열의 길이
    arr.push(4, 5, 6);
    ...
    const obj = {};
    expect(Object.keys(obj).length).to.equal(0); //객체의 속성의 개수가 0개와 같다
    obj['name'] = 'codestates';
    obj.quality = 'best';
  ...
  it('참조 자료형을 변수에 할당할 경우, 데이터의 주소가 저장됩니다.', function () {
    ...
    */
    ...
    const ages = [22, 23, 27];
    allowedToDrink = ages;
    expect(allowedToDrink === ages).to.equal(true); //주소를 복사해서 동일한 데이터를 바라보게 만든다.
    expect(allowedToDrink === [22, 23, 27]).to.equal(false);//age참조한주소 vs 일반배열주소

    const nums1 = [1, 2, 3];
    const nums2 = [1, 2, 3];
    expect(nums1 === nums2).to.equal(false);//일반 num주소1 vs일반 num주소2
    ...
    expect(person.son === boy).to.equal(true); //boy의 주소는 person.son의 주소와 동일하다
    expect(person.son === { age: 9 }).to.equal(false); //age는 현재 20이고 일반 age배열주소와 person.son의 주소는 다르다.
    expect(person.son === { age: 20 }).to.equal(false); //일반 age배열주소와 person.son의 주소는 다르다.
