describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    ...
    expect(emptyObj.length).to.equal(undefined); //객체의 길이는 없다
    ...
    expect(megalomaniac.length).to.equal(undefined); //객체의 속성의 개수는 이렇게 세는 것이 아니다, 객체 속성의 길이는 없다.
  ...
  it('Object를 함수의 전달인자로 전달할 경우, reference가 전달됩니다.', function () {
    ...
    const copiedObj = Object.assign({}, obj); //Object.assign(빈것으로부터 현재obj를 채워넣는다)
    copiedObj.mastermind = 'James Wood'; //이제copyobj는 obj를 참조하지않는다. 다른 주소이다
    expect(obj.mastermind).to.equal('Joker');

    obj.henchwoman = 'Harley';//이제copyobj는 obj를 참조하지않는다. 다른 주소이다
    expect(copiedObj.henchwoman).to.equal('Adam West');//copyobj를 만들당시의 obj상태를 반영한다.
